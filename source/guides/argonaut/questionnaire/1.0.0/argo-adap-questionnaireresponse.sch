...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <!-- 
    This file contains just the constraints for the profile argo_questionnaireresponse
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:QuestionnaireResponse">
      ...
      <sch:assert test="count(f:extension[@url = 'http://fhir.org/guides/argonaut/questionnaire/StructureDefinition/extension-responsePeriod']) &lt;= 1">extension with URL = 'http://fhir.org/guides/argonaut/questionnaire/StructureDefinition/extension-responsePeriod': maximum cardinality of 'extension' is 1</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:title>QuestionnaireResponse.contained</sch:title>
    <sch:rule context="f:QuestionnaireResponse/f:contained">
      <sch:assert test="not(parent::f:contained and f:contained)">If the resource is contained in another resource, it SHALL NOT contain nested Resources (inherited)</sch:assert>
      <sch:assert test="not(parent::f:contained and f:text)">If the resource is contained in another resource, it SHALL NOT contain any narrative (inherited)</sch:assert>
      <sch:assert test="not(exists(f:contained/*/f:meta/f:versionId)) and not(exists(f:contained/*/f:meta/f:lastUpdated))">If a resource is contained in another resource, it SHALL NOT have a meta.versionId or a meta.lastUpdated (inherited)</sch:assert>
      <sch:assert test="not(exists(for $id in f:contained/*/@id return $id[not(ancestor::f:contained/parent::*/descendant::f:reference/@value=concat('#', $id))]))">If the resource is contained in another resource, it SHALL be referred to from elsewhere in the resource (inherited)</sch:assert>
